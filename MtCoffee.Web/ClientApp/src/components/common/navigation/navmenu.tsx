import { RateLimiter } from '@root/utils/rateLimiter';
import { forceUpdateAsyncFactory, setStateAsyncFactory } from '@root/utils/stateSetter';
...
export class NavMenu extends React.PureComponent<NavMenuProps, NavMenuState> {
    ...
    private onRenderNavMenuItem(itemProps: TreeNode<TreeNodeMetaData>, isCurrentNavChild: boolean): React.ReactNode {
        ...
        const { isExpanded, text, to, onClick } = val;
        ...
        if (children.length > 0) {
            ...
            return (<li className='f-nav-link' key={uniqueId}>
                <button tabIndex={tabIndex} onClick={onClick}>{text}</button>
            ...
            throw new Error('The \'to\' property must be defined, or else sub child items must be given.');
        }
    }
